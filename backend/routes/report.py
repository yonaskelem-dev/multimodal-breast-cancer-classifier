...
def generate_pdf(data: ReportRequest):
    ...
    # Plain hex strings for use inside ReportLab XML markup
    result_hex = "#DC2626" if data.prediction == "MALIGNANT" else "#059669"
    ...
    result_data = [
        [
            Paragraph(
                f'<b><font size="16" color="{result_hex}">{data.prediction}</font></b>',
                ParagraphStyle("r", alignment=TA_CENTER)
            ),
            Paragraph(
                f'<b>Confidence: {data.confidence:.1f}%</b>',
    ...
    disclaimer = (
        "<b>Medical Disclaimer:</b> This report is "
